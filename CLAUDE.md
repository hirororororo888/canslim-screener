...
- Minervini（8/2 0:37、8/3時点で最新のまま更新なし）: 内容は前回記録と同一（$SPYショートヘッジ継続、10年債5%警戒、FTD待ち）。8/1投稿はMinervini SPY Allocation Modelの宣伝のみでスキップ
...
- IBD推奨の現物株保有比率は0-20%のまま（現金80-100%推奨、変更報告なし）
...
- 推奨ポジション: **キャッシュ最優先（80-100%）を継続、ただしフォロースルーデイの出現に備えてウォッチリスト（GE/NMM/VIK/NTAP + LLY + Checker検証済み銘柄）を準備**。正式なFTD確認・トレンド上方修正までは新規個別株購入は見送り
